...
test("public package exposes thin wrapper surface", function()
  ...
  assert_equal("run,usage", table.concat(keys, ","))
  assert_contains(mutate4lua.usage(), "mutate4lua <file.lua>")
end)

test("public run delegates directly to the Lua CLI", function()
  local root = temp_dir()
  write(util.join_path(root, "sample.lua"), "return true\n")

  local out = buffer()
  local err = buffer()
  local exit = mutate4lua.run({"sample.lua", "--scan"}, { cwd = root, stdout = out, stderr = err })
  assert_equal(0, exit)
  assert_contains(out:text(), "sites:")
